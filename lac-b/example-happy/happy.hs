...
happyReduction_1 (_ `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Int happy_var_3)) `HappyStk`
	(HappyTerminal (Var happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn4
		 (Leta happy_var_2 happy_var_3
	) `HappyStk` happyRest
...
happyNewToken action sts stk (tk:tks) =
	...
	Let -> cont 5;
	In -> cont 6;
	Sym happy_dollar_dollar -> cont 7;
	Var happy_dollar_dollar -> cont 8;
	Int happy_dollar_dollar -> cont 9;
	_ -> happyError' (tk:tks)
	}
...
kevin tks = happyRunIdentity happySomeParser where
