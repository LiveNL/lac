...
spacePrinter s = pos' last' ++ printer (L.toList s)
  where last' = last (L.toList s)
        pos' ((x, y), _) = "(" ++ show x ++ "," ++ show y ++ ")\n"

printer :: Eq a => [((a, b), Contents)] -> [Char]
printer [((x,y), z)] = content' z
printer (((x,y), z):k@((a,b), c):xs) | x == a    = content' z ++ printer (k: xs)
                                     | otherwise = content' z ++ "\n" ++ printer (k:xs)

content' :: Contents -> String
content' c = [(fromJust (lookup c contentsTable))]
